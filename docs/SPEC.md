...
只更新持久化依赖目录中的兼容版本，并通过平台重启切换代码。平台每天只读检查一次兼容组件版本及当前
已启用插件代码明确使用的内核通道，并可在启动后异步检查；只有 CloakBrowser 已选中、免费 Key 模式开启且
Key 非空时才允许发起检查请求，任一条件不满足都必须在本地静默跳过。定时检查不得自动安装或替换组件、
不得下载内核；CloakBrowser 自带的启动时后台更新由平台关闭。管理员显式点击“检查并更新”时，平台必须
更新组件并只把缺失的插件所需最新内核下载到持久化缓存，完成后重启平台。不得仅因某个内核目录已存在
就继续更新该通道；停用或删除插件后，不再为其预下载内核。固定具体浏览器版本或动态决定通道的插件由
实际调用自行下载精确内核，平台不得猜测。首次没有可用内核时，实际浏览器调用仍可下载所需内核。
