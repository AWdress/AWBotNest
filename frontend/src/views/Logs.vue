...
const logTime = formatLogTime

const filtered = computed(() => {
  return logs.value.filter((l) => {
    if (!isDisplayLog(l)) return false
...
function connect() {
  disconnect()
  ws = new WebSocket(wsUrl(), ['awbotnest', `auth.${getToken()}`])
      ...
      <div class="row gap">
        ...
        <div class="level-tabs" aria-label="日志级别筛选">
          <button v-for="lv in levels" :key="lv" type="button"
                  ...
                  @click="levelFilter = lv">{{ logLevelLabel(lv) }}</button>
        </div>
...
.level { flex-shrink: 0; width: 54px; font-weight: 600; }
.lv-debug { color: var(--text-muted); }
.lv-info { color: var(--accent); }
.lv-warn { color: var(--warning); }
.lv-err { color: var(--danger); }
.source { width: 112px; overflow: hidden; color: var(--accent-2); flex-shrink: 0; text-overflow: ellipsis; white-space: nowrap; }
