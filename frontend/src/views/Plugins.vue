...
function useFallbackPluginIcon(event) {
  ...
  image.src = logo
  image.classList.add('store-icon-fallback', 'config-plugin-icon-fallback', 'fallback')
}
...
const pluginLogTime = formatLogTime

function matchesPlugin(item, id) {
  // 插件 ctx.log 使用 awbotnest.plugin.<id>；兼容旧版直接使用插件 id 的记录。
  return item.source === `awbotnest.plugin.${id}` || item.source === id
    || (item.msg && item.msg.startsWith(`[${id}]`))
}
...
function logsWsUrl() {
  ...
  return `${proto}://${location.host}/api/logs/ws`
}

function logsConnect() {
  logsWs = new WebSocket(logsWsUrl(), ['awbotnest', `auth.${getToken()}`])
...
const pluginTags = (plugin) => (Array.isArray(plugin?.tags) ? plugin.tags : [])
  .map((tag) => String(tag || '').trim()).filter(Boolean).slice(0, 4)
const pluginAuthor = (plugin) => plugin?.author || shortRepo(plugin?.repo_url || '').split('/')[0] || '未知作者'
        ...
        <div v-for="p in filteredPlugins" :key="p.id" class="card plugin-card clickable"
          ...
          <div class="card-head">
            <div class="store-title">
              <img :src="p.icon || logo" class="store-icon" :class="{ 'store-icon-fallback': !p.icon }"
                   :alt="`${p.name || p.id} Logo`" @error="useFallbackPluginIcon" />
          ...
          <div class="card-meta">
            <span class="github-author" :title="`作者：${pluginAuthor(p)}`">
              ...
              <div class="card-head">
                <div class="store-title">
                  <img :src="p.icon || logo" class="store-icon" :class="{ 'store-icon-fallback': !p.icon }"
                       :alt="`${p.name || p.id} Logo`" @error="useFallbackPluginIcon" />
                  <div class="card-title">
                    <span class="name"><span class="name-text">{{ p.name }}</span>
                      <span class="plugin-version">v{{ p.local_version }} → v{{ p.version }}</span>
                      <span v-if="isOfficial(p)" class="badge-official">官方</span>
                    </span>
                    <span class="badge badge-update">发现新版本</span>
                  </div>
                </div>
              </div>
      ...
      <div class="search-modal card" role="dialog" aria-modal="true" aria-label="搜索插件">
        ...
        <div class="search-list">
          ...
          <div v-for="(p, index) in searchResults" :key="p.id" class="search-item"
            ...
            <img :src="p.icon || logo" class="search-icon" :class="{ fallback: !p.icon }"
                 :alt="`${p.name || p.id} Logo`" @error="useFallbackPluginIcon" />
            ...
            <div v-for="node in dependencyData.nodes" :key="node.id" class="dependency-node">
              ...
              <div class="dependency-links">
                <span v-for="edge in dependencyData.edges.filter(item => item.from === node.id)" :key="`${edge.type}-${edge.to}`" :class="{ missing: edge.missing }">
                  {{ edge.type === 'provides' ? '提供能力 → ' : edge.type === 'capability' ? '需要能力 → ' : edge.type === 'python' ? 'Python 包 → ' : '依赖插件 → ' }}{{ edge.label || edge.to }}<template v-if="edge.missing">（缺失）</template>
                </span>
                <span v-if="!dependencyData.edges.some(item => item.from === node.id)" class="muted">未声明依赖</span>
              </div>
            </div>
          </template>
        </div>
        ...
        <div class="modal-body">
          <div class="changelog-header">
            <div class="changelog-plugin-info">
              <img :src="changelogTarget.icon || logo" class="changelog-icon"
                   :class="{ 'changelog-icon-fallback': !changelogTarget.icon }"
                   :alt="`${changelogTarget.name || changelogTarget.id} Logo`" @error="useFallbackPluginIcon" />
              <div>
...
.github-author svg { width: 17px; height: 17px; flex: 0 0 17px; }
.github-author span { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.heat-count {
