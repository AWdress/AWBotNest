...
async function testProxy() {
  ...
  try {
    const result = await api.testProxy(s.value.proxy_set)
    proxyTest.value = { ...result, message: result.message || result.detail || (result.ok ? '代理连接正常' : '代理连接失败') }
  }
      ...
      <div v-if="tab === 'system'" class="card" style="margin-top:16px">
        ...
        <div class="grid2">
          <div class="field"><label>类型</label>
            <select class="select" v-model="s.DB_INFO.dbset" disabled>
