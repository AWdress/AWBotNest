...
import { isDisplayLog, logMessageLabel, logSourceLabel } from '../utils/logDisplay'
...
function jobName(job) { return jobDisplayName(job) }
...
function eventTitle(source) { return EVENT_SOURCE_NAMES[source] || logSourceLabel(source, st.value?.plugin_names) }
...
function normalizeEvent(item) {
  ...
  return {
    key: eventKey(item),
    source,
    detail: logMessageLabel(item.msg),
...
function addEvent(item) {
  if (!item || item.type === 'history' || !isDisplayLog(item)) return
...
function connectEvents() {
  ...
  eventSocket = new WebSocket(
    `${protocol}://${location.host}/api/logs/ws`,
    ['awbotnest', `auth.${getToken()}`],
  )
  ...
  eventSocket.onmessage = message => {
    try {
      ...
      if (item.type === 'history' && Array.isArray(item.logs)) {
        events.value = item.logs.filter(isDisplayLog).slice(0, 6).map(normalizeEvent)
        return
      }
      addEvent(item)
    } catch {}
...
.job-progress i { display: block; width: 100%; height: 100%; border-radius: inherit; background: var(--success); transform-origin: left; transition: transform .25s ease; }
