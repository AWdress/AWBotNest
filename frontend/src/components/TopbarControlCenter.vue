...
import { jobDisplayName, jobOwnerLabel } from '../utils/jobDisplay'
...
const filteredLogs = computed(() => logs.value.filter(item => {
  if (!isDisplayLog(item)) return false
...
async function runJob(job) {
  ...
  try {
    ...
    toast.success(`已开始执行：${jobDisplayName(job)}`)
        ...
        <div v-if="modal === 'logs'" class="modal-body logs-modal" ref="quickLogsBox">
          <div class="log-level-tabs" aria-label="日志级别筛选">
            <button v-for="level in logLevels" :key="level" type="button"
                    ...
                    @click="logLevel = level">{{ logLevelLabel(level) }}</button>
          </div>
          ...
          <div class="quick-log-list">
            ...
            <div v-for="(item, index) in filteredLogs" :key="item.id || `${item.timestamp}-${index}`" class="quick-log-row">
              ...
              <b :class="`level-${item.level?.toLowerCase()}`">{{ logLevelLabel(item.level) }}</b>
              <span><small class="quick-log-source">{{ logSourceLabel(item.source) }}</small>{{ logMessageLabel(item.msg) }}</span>
            </div>
          </div>
        </div>
...
.quick-log-source { display: inline-block; margin-right: 8px; color: var(--accent-2); font-family: var(--font-sans); }
...
@media (max-width: 768px) {
  ...
  .control-btn, .avatar-btn { width: 40px; height: 40px; }
  .control-center > .profile-trigger {
    width: 40px; min-width: 40px; max-width: 40px; min-height: 40px; padding: 0;
    flex: 0 0 40px; border-radius: 50%;
  }
  .control-center > .profile-trigger .avatar-btn { width: 40px; height: 40px; flex-basis: 40px; }
  .profile-trigger .profile-trigger-copy, .profile-trigger .profile-chevron { display: none; }
  .control-pop { position: fixed; top: calc(54px + env(safe-area-inset-top) + 8px); right: 8px; width: calc(100vw - 16px); max-width: calc(100vw - 16px); max-height: calc(100dvh - 148px - env(safe-area-inset-top)); box-sizing: border-box; }
  ...
  .control-modal > header { padding: 12px max(14px, env(safe-area-inset-right)) 12px max(14px, env(safe-area-inset-left)); }
  .control-modal > header button { width: 44px; height: 44px; }
  .control-modal .modal-body { padding: 14px max(14px, env(safe-area-inset-right)) calc(14px + env(safe-area-inset-bottom)) max(14px, env(safe-area-inset-left)); }
