<div align="center">
  ...
  一个 Telegram 机器人管理工具。所有功能都是**插件**——在管理页面里点几下就能安装、开关，不用懂代码、不用重启。
</div>
...
2. **填 Telegram 凭据**：到「系统设置 → Telegram 凭据」，填入从 [my.telegram.org](https://my.telegram.org) 申请的 `API_ID` / `API_HASH`，机器人功能再填 [@BotFather](https://t.me/BotFather) 给的 `BOT_TOKEN`。保存后重启生效。
...
- **系统设置**：登录密码、Telegram 凭据、网页服务、代理、数据库和插件仓库地址。
...
**插件报错/标红？** 在插件卡片上能看到错误原因，多半是插件本身的问题，删掉重装或换一个即可，不影响系统和其他插件。
